...
class CNNModel(nn.Module):
    def __init__(self, max_features, embedding_dim, dropout_rate):
        ...
        self.conv_layer = nn.Conv1d(in_channels=self.embedding_dim, out_channels=1000, kernel_size=2, padding=1)
        self.relu = nn.ReLU()
        self.flatten = nn.Flatten()
        self.dropout_layer = nn.Dropout(dropout_rate)
        self.dense_layer_1 = nn.Linear(1000*(max_features+1), 100)     
        self.dense_layer_2 = nn.Linear(100, 1)   
        self.sigmoid = nn.Sigmoid()

    def forward(self, X):
        embedded = self.embedding_layer(X)
        conv_output = self.conv_layer(embedded.permute(0, 2, 1))
        out = self.relu(conv_output)
        out = self.dropout_layer(out)
        out = self.flatten(out)
        out = self.relu(self.dense_layer_1(out))
        out = self.sigmoid(self.dense_layer_2(out))
        return out.squeeze()
